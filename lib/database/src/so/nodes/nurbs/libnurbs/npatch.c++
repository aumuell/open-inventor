...
{
   void* p = calloc( (MAXORDER * MAXORDER * MAXCOORDS), sizeof(REAL) );
   if ( ! p ) {
      fprintf(stderr,"Bad calloc in Patch::alloc_pts(); exiting.\n");
      exit(1);
   }
   return (REAL*) p;
}

void Patch::alloc_all_pts( void )
{
   cpts = alloc_pts();
   bpts = alloc_pts();
   spts = alloc_pts();
...
Patch::Patch( Quilt_ptr geo, REAL *pta, REAL *ptb, Patch *n )
{
    ...
    pspec[0].stepsize = 0.0;
    pspec[1].stepsize = 0.0;
    pspec[0].minstepsize = 0.0;
    pspec[1].minstepsize = 0.0;
...
Patch::Patch( Patch& upper, int param, REAL value, Patch *n )
{
    ...
    lower.pspec[0].stepsize = 0.0;
    lower.pspec[1].stepsize = 0.0;
    lower.pspec[0].minstepsize = 0.0;
    lower.pspec[1].minstepsize = 0.0;
    lower.pspec[0].needsSubdivision = 0;
    lower.pspec[1].needsSubdivision = 0;
    lower.next = n;
...
void
Patch::copy( Patch *fromPatch )
{
    // Copy each field over from the fromPatch instance
    mapdesc        = fromPatch->mapdesc;
    cullval        = fromPatch->cullval;
    notInBbox      = fromPatch->notInBbox;
    needsSampling  = fromPatch->needsSampling;
    pspec[0]       = fromPatch->pspec[0];
    pspec[1]       = fromPatch->pspec[1];
 
    pspec[0].stepsize = fromPatch->pspec[0].stepsize;
    pspec[1].stepsize = fromPatch->pspec[1].stepsize;
    pspec[0].minstepsize = fromPatch->pspec[0].minstepsize;
    pspec[1].minstepsize = fromPatch->pspec[1].minstepsize;
...
Patch::getstepsize( void )
{
	...
	int val = mapdesc->project( spts, pspec[0].stride, pspec[1].stride, 
		...
		if( ss != 0.0 && tt != 0.0 ) {
		    ...
		    REAL ttq = ::sqrtf( (float) tt );
		    REAL ssq = ::sqrtf( (float) ss );
		    ...
		    REAL scutoff = 2.0 * t / ( pspec[0].range[2] * pspec[0].range[2]);
		    ...
		    REAL tcutoff = 2.0 * t / ( pspec[1].range[2] * pspec[1].range[2]);
		    ...
		    if( 4.0 * t  > st * pspec[0].range[2] * pspec[1].range[2] ) {
			pspec[0].singleStep();
			pspec[1].singleStep();
		    } else {
			REAL area = 4.0 * t / st;
