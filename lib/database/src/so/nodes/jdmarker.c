...
/* Process a SOFn marker */
{
  ...
  if ((int32_t) cinfo->image_height > (int32_t) JPEG_MAX_DIMENSION ||
      (int32_t) cinfo->image_width > (int32_t) JPEG_MAX_DIMENSION)
...
#define JFIF_LEN 14
  ...
  if (length > 0)		/* skip any remaining data -- could be lots */
    (*cinfo->src->skip_input_data) (cinfo, (int32_t) length);
...
/* Skip over an unknown or uninteresting variable-length marker */
{
  ...
  (*cinfo->src->skip_input_data) (cinfo, (int32_t) length - 2L);
