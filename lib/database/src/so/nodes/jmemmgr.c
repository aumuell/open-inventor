...
/*
 * Many machines require storage alignment: int32_ts must start on 4-byte
 ...
 * aligned well.  Put "#define ALIGN_TYPE int32_t" in jconfig.h if you have
 * such a compiler.
 */
...
print_mem_stats (j_common_ptr cinfo, int pool_id)
{
   ...
   * This is helpful because message parm array can't handle int32_ts.
   */
  ...
  for (lhdr_ptr = mem->large_list[pool_id]; lhdr_ptr != NULL;
    ...
    fprintf(stderr, "  Large chunk used %ld\n",
	    (int32_t) lhdr_ptr->hdr.bytes_used);
  }

  for (shdr_ptr = mem->small_list[pool_id]; shdr_ptr != NULL;
       shdr_ptr = shdr_ptr->hdr.next) {
    fprintf(stderr, "  Small chunk used %ld free %ld\n",
	    (int32_t) shdr_ptr->hdr.bytes_used,
	    (int32_t) shdr_ptr->hdr.bytes_left);
  }
}
 ...
 * machines, but may be too small if int32_ts are 64 bits or more.
 */
...
/* Allocate a 2-D sample array */
{
  ...
  ltemp = (MAX_ALLOC_CHUNK-SIZEOF(large_pool_hdr)) /
	  ((int32_t) samplesperrow * SIZEOF(JSAMPLE));
  if (ltemp <= 0)
    ERREXIT(cinfo, JERR_WIDTH_OVERFLOW);
  if (ltemp < (int32_t) numrows)
...
/* Allocate a 2-D coefficient-block array */
{
  ...
  /* Calculate max # of rows allowed in one allocation chunk */
  ltemp = (MAX_ALLOC_CHUNK-SIZEOF(large_pool_hdr)) /
	  ((int32_t) blocksperrow * SIZEOF(JBLOCK));
  if (ltemp <= 0)
    ERREXIT(cinfo, JERR_WIDTH_OVERFLOW);
  if (ltemp < (int32_t) numrows)
    rowsperchunk = (JDIMENSION) ltemp;
...
/* Request a virtual 2-D sample array */
{
  ...
  numrows = (JDIMENSION) jround_up((int32_t) numrows, (int32_t) unitheight);
...
{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  int32_t space_per_unitheight, maximum_space, avail_mem;
  int32_t unitheights, max_unitheights;
  ...
  for (sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next) {
    if (sptr->mem_buffer == NULL) { /* if not realized yet */
      space_per_unitheight += (int32_t) sptr->unitheight *
			      (int32_t) sptr->samplesperrow * SIZEOF(JSAMPLE);
      maximum_space += (int32_t) sptr->rows_in_array *
		       (int32_t) sptr->samplesperrow * SIZEOF(JSAMPLE);
    }
  }
  for (bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next) {
    if (bptr->mem_buffer == NULL) { /* if not realized yet */
      space_per_unitheight += (int32_t) bptr->unitheight *
			      (int32_t) bptr->blocksperrow * SIZEOF(JBLOCK);
      maximum_space += (int32_t) bptr->rows_in_array *
		       (int32_t) bptr->blocksperrow * SIZEOF(JBLOCK);
    }
  }
  ...
  for (sptr = mem->virt_sarray_list; sptr != NULL; sptr = sptr->next) {
    if (sptr->mem_buffer == NULL) { /* if not realized yet */
      unitheights = ((int32_t) sptr->rows_in_array - 1L) / sptr->unitheight + 1L;
	...
	jpeg_open_backing_store(cinfo, & sptr->b_s_info,
				(int32_t) sptr->rows_in_array *
				(int32_t) sptr->samplesperrow *
				(int32_t) SIZEOF(JSAMPLE));
	sptr->b_s_open = TRUE;
      }
  ...
  for (bptr = mem->virt_barray_list; bptr != NULL; bptr = bptr->next) {
    if (bptr->mem_buffer == NULL) { /* if not realized yet */
      unitheights = ((int32_t) bptr->rows_in_array - 1L) / bptr->unitheight + 1L;
      if (unitheights <= max_unitheights) {
	/* This buffer fits in memory */
	...
	jpeg_open_backing_store(cinfo, & bptr->b_s_info,
				(int32_t) bptr->rows_in_array *
				(int32_t) bptr->blocksperrow *
				(int32_t) SIZEOF(JBLOCK));
	bptr->b_s_open = TRUE;
      }
...
{
  int32_t bytesperrow, file_offset, byte_count, rows, i;

  bytesperrow = (int32_t) ptr->samplesperrow * SIZEOF(JSAMPLE);
  file_offset = ptr->cur_start_row * bytesperrow;
  /* Loop to read or write each allocation chunk in mem_buffer */
  for (i = 0; i < (int32_t) ptr->rows_in_mem; i += ptr->rowsperchunk) {
    /* One chunk, but check for short chunk at end of buffer */
    rows = MIN((int32_t) ptr->rowsperchunk, (int32_t) ptr->rows_in_mem - i);
    /* Transfer no more than fits in file */
    rows = MIN(rows, (int32_t) ptr->rows_in_array -
		    ((int32_t) ptr->cur_start_row + i));
...
/* Do backing store read or write of a virtual coefficient-block array */
{
  ...
  bytesperrow = (int32_t) ptr->blocksperrow * SIZEOF(JBLOCK);
  file_offset = ptr->cur_start_row * bytesperrow;
  /* Loop to read or write each allocation chunk in mem_buffer */
  for (i = 0; i < (int32_t) ptr->rows_in_mem; i += ptr->rowsperchunk) {
    /* One chunk, but check for short chunk at end of buffer */
...
/* caller intends to modify the accessed area. */
{
  ...
  if (start_row < ptr->cur_start_row ||
    ...
    if (start_row > ptr->cur_start_row) {
      ...
      /* use int32_t arithmetic here to avoid overflow & unsigned problems */
      int32_t ltemp;

      ltemp = (int32_t) start_row + (int32_t) ptr->unitheight -
	      (int32_t) ptr->rows_in_mem;
      if (ltemp < 0)
...
jinit_memory_mgr (j_common_ptr cinfo)
{
  ...
  if ((int32_t) test_mac != MAX_ALLOC_CHUNK ||
