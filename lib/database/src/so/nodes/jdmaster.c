...
/* Do computations that are needed before master selection phase */
{
  ...
  if (cinfo->scale_num * 8 <= cinfo->scale_denom) {
    ...
    cinfo->output_width = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_width, 8);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_height, 8);
    cinfo->min_DCT_scaled_size = 1;
  } else if (cinfo->scale_num * 4 <= cinfo->scale_denom) {
    /* Provide 1/4 scaling */
    cinfo->output_width = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_width, 4);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_height, 4);
    cinfo->min_DCT_scaled_size = 2;
  } else if (cinfo->scale_num * 2 <= cinfo->scale_denom) {
    /* Provide 1/2 scaling */
    cinfo->output_width = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_width, 2);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_height, 2);
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    compptr->width_in_blocks = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_width * (int32_t) compptr->h_samp_factor,
		    (int32_t) (cinfo->max_h_samp_factor * DCTSIZE));
    compptr->height_in_blocks = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_height * (int32_t) compptr->v_samp_factor,
		    (int32_t) (cinfo->max_v_samp_factor * DCTSIZE));
    /* Size in samples, after IDCT scaling */
    compptr->downsampled_width = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_width *
		    (int32_t) (compptr->h_samp_factor * compptr->DCT_scaled_size),
		    (int32_t) (cinfo->max_h_samp_factor * DCTSIZE));
    compptr->downsampled_height = (JDIMENSION)
      jdiv_round_up((int32_t) cinfo->image_height *
		    (int32_t) (compptr->v_samp_factor * compptr->DCT_scaled_size),
		    (int32_t) (cinfo->max_v_samp_factor * DCTSIZE));
...
master_selection (j_decompress_ptr cinfo)
{
  ...
  samplesperrow = (int32_t) cinfo->output_width * (int32_t) cinfo->out_color_components;
  jd_samplesperrow = (JDIMENSION) samplesperrow;
  if ((int32_t) jd_samplesperrow != samplesperrow)
